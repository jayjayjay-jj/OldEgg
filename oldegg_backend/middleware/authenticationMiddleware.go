...
func ShopMiddleware(ctx *gin.Context) {
	type JWTToken struct {
		TokenString string `json:"token_string"`
	}
	...
	if claims, ok := result.Claims.(jwt.MapClaims); ok && result.Valid {
		...
		var currShop model.Shop
		config.DB.Model(model.Shop{}).Where("email = ?", claims["subject"]).First(&currShop)

		if currShop.Status == "Banned" {
			ctx.JSON(200, "You can't sign in")
			return
		}

		ctx.Set("currentShop", currShop)
		ctx.Next()
	} else {
		fmt.Println(error)
	}
